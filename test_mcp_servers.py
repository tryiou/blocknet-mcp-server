...
async def safe_call(session, tool_name, params, expected_type, context=None):
    """
...
async def discover_orders(session, context):
    ...
    success, data, error = await safe_call(session, "dxGetOrders", {}, "list")
...
async def discover_block_data(session, context):
    ...
    # Get block count
    success, data, error = await safe_call(session, "xrGetBlockCount", 
    ...
    if success and isinstance(data, dict):
        ...
        if height:
            ...
            success2, data2, error2 = await safe_call(session, "xrGetBlockHash",
...
async def discover_utxos(session, context):
    ...
    success, data, error = await safe_call(session, "dxGetUtxos",
